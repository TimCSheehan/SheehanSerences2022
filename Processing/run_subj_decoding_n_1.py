...
def run_subj(subj,rois,t_points,sav_root,just_IEM=0,paramsT={'donut':50,'anovaP':50}, paramsL={'donut':75,'anovaP':99},nb=1):
    D = DNC.noiseModeling()
    if just_IEM:
        CV_fun = D.fit_CV_IEM
    else:
...
#             dat_loc_train = dat_loc_train0[:,donut_ev].T
            ...
            if nb==1:
                ori_n_1 = np.concatenate([[np.nan],ori_og[:-1]])
                bad_ev = np.isin(ev_task_og.sess,bad_sess)
                bad_trials = np.any(np.isnan(dat_task),0) | (ev_task_og.trial==1)
            elif nb==-1:
                ori_n_1 = np.concatenate([ori_og[1:],[np.nan]])
                bad_ev = np.isin(ev_task_og.sess,bad_sess)
                bad_trials = np.any(np.isnan(dat_task),0) | (ev_task_og.trial==17)
            ...
            ori_task = ori_n_1[~bad_ev&~bad_trials] # setup n-1 ori
...
#             donut_ev2 = (np.concatenate(([0],ev_loc.d1.values[:-1]))==1) & (ev_loc.d1.values==1)
            ...
            ev_loc_loc = ev_loc[donut_use]
#             ori_loc_loc = D.foldPred(ev_loc_loc.d0.values.copy())
            dat_loc_loc = dat_loc_train1[:,donut_use].T
            G_loc = ev_loc_loc.sess.values
            fit_locLoc = pd.Series(CV_fun(dat_loc_loc,ori_loc,G_loc))
            fit_locLoc['ev'] = ev_loc_loc
            fit_task['ori_loc'] = ori_loc
