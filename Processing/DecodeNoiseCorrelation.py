...
class noiseModeling:
    ...
    def __init__(self,shut_up=0):
        ...
        self.get_full_IEM_out = True # for correcting baseline shifts offline...
    ...
    def decodeIEM(self,test_samples):
        ...
        if self.get_full_IEM_out:
            return self.foldPred(est),unc, IEM
...
#         estInit = self.foldPred(estInit)
    ...
    def fit_CV_IEM(self,samples,ori,G):
        ...
        IEM = np.zeros((n_trials_total,self.nchans))
        ...
        for g in np.unique(G):
            ...
            this_out = self.decodeIEM(samples_test)
            if self.get_full_IEM_out:
                _est,_unc,_IEM = this_out 
                IEM[G==g] = _IEM
            else:
                _est,_unc = this_out
        ...
        unc=unc[G!=-1]
        IEM=IEM[G!=-1]
        dat = {'ori':ori[G!=-1],'estIEM':est,'uncIEM':unc,'IEM':IEM}
        return dat
